...
#编译 libdatrie 插件
RUN mkdir -pv /opt/soft && cd /opt/soft && wget -c -nv  https://linux.thai.net/pub/ThaiLinux/software/libthai/libdatrie-0.2.11.tar.xz && tar -zxf libdatrie-0.2.11.tar.xz && cd libdatrie-0.2.11 && ./configure --prefix=/usr/local && make && make install && rm -rf /opt/soft
