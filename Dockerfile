...
#敏感词过滤
RUN mkdir -pv /opt/soft && cd /opt/soft && git clone https://github.com/preakrel/libdatrie.git &&  cd libdatrie && ./configure  --prefix=/usr/local/libdatrie && make && make install && rm -rf /opt/soft && rm -rf  /var/tmp/* /tmp/*
