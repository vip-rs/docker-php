...
#敏感词过滤
RUN mkdir -pv /opt/soft && cd /opt/soft && wget -c -nv https://linux.thai.net/pub/thailinux/software/libthai/libdatrie-0.2.13.tar.xz && xz -d libdatrie-0.2.13.tar.xz && cd  libdatrie-0.2.13 && ./configure  --prefix=/usr/local/libdatrie && make && make install && rm -rf /opt/soft
