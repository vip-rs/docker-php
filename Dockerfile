...
#编译 jemalloc
RUN mkdir -pv /opt/soft && cd /opt/soft && wget -c -nv https://github.com/jemalloc/jemalloc/releases/download/4.5.0/jemalloc-4.5.0.tar.bz2 && tar -jxf jemalloc-4.5.0.tar.bz2 && cd jemalloc-4.5.0/ && ./configure --with-jemalloc-prefix=je_ --prefix=/usr/local/jemalloc && make && make install && rm -rf /opt/soft
...
#敏感词过滤
RUN mkdir -pv /opt/soft && cd /opt/soft && wget -c -nv ftp://linux.thai.net/pub/ThaiLinux/software/libthai/libdatrie-0.2.5.tar.gz && tar -zxf libdatrie-0.2.5.tar.gz && cd  libdatrie-0.2.5 && ./configure  --prefix=/usr/local/libdatrie && make && make install && rm -rf /opt/soft && rm -rf  /var/tmp/* /tmp/*
