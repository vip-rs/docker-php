# Version 0.3
# 基础镜像
FROM daocloud.io/php_ity/doc-php:php8.0.3
...
#编译 redis 插件
RUN mkdir -pv /opt/soft && cd /opt/soft && wget -c -nv  http://pecl.php.net/get/redis-5.3.2.tgz && tar -zxf redis-5.3.2.tgz && cd redis-5.3.2 && /usr/local/php/bin/phpize && ./configure --with-php-config=/usr/local/php/bin/php-config && make && make install && rm -rf /opt/soft

#编译 event 插件
RUN mkdir -pv /opt/soft && cd /opt/soft && wget -c -nv  http://pecl.php.net/get/event-3.0.0.tgz && tar -zxf event-3.0.0.tgz && cd event-3.0.0 && /usr/local/php/bin/phpize && ./configure   --with-php-config=/usr/local/php/bin/php-config --with-event-core --with-event-extra && make && make install && rm -rf /opt/soft

#编译 yaml 插件
RUN mkdir -pv /opt/soft && cd /opt/soft && wget -c -nv  http://pecl.php.net/get/yaml-2.2.0.tgz && tar -zxf yaml-2.2.0.tgz && cd yaml-2.2.0 && /usr/local/php/bin/phpize && ./configure  --with-php-config=/usr/local/php/bin/php-config  && make && make install && rm -rf /opt/soft

#编译 msgpack 插件
RUN mkdir -pv /opt/soft && cd /opt/soft && wget -c -nv  http://pecl.php.net/get/msgpack-2.1.1.tgz && tar -zxf  msgpack-2.1.1.tgz && cd msgpack-2.1.1 && /usr/local/php/bin/phpize && ./configure  --with-php-config=/usr/local/php/bin/php-config  && make && make install && rm -rf /opt/soft
...
#编译 mongodb 插件
RUN mkdir -pv /opt/soft && cd /opt/soft && wget -c -nv  http://pecl.php.net/get/mongodb-1.8.2.tgz && tar -zxf mongodb-1.8.2.tgz && cd mongodb-1.8.2 && /usr/local/php/bin/phpize && ./configure  --with-php-config=/usr/local/php/bin/php-config  && make && make install && rm -rf /opt/soft
...
#编译 swoole
RUN mkdir -pv /opt/soft && cd /opt/soft && wget -c  https://github.com/swoole/swoole-src/archive/v4.5.0.tar.gz && tar -zxf v4.5.0.tar.gz  && cd swoole-src-4.5.0  &&  /usr/local/php/bin/phpize && ./configure  --enable-sockets  --enable-openssl  --with-openssl-dir=/usr/local/openssl    --enable-http2  --with-nghttp2-dir=/usr/local/include/nghttp2   --enable-mysqlnd   --enable-coroutine-postgresql  --enable-debug-log  --enable-trace-log   --with-php-config=/usr/local/php/bin/php-config && make && make install && rm -rf /opt/soft
